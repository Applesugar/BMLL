...
{
    kmeans k(4);
    vector<vector<double>>samples(80, vector<double>(2, 0));
    vector<vector<double>>centriods;
    fstream A;
    A.open("D:\\sample.txt", ios::in);
    if (!A.is_open()) {
        cout << "No Such Path or Training Text!" << endl;
        return -1;
    }
    ...
    A.close();
    vector<int>lables(samples.size(), 0);
    centriods = k.Itera_Compute(samples, lables);
    for (int i = 0; i < centriods.size(); i++)
    {
        for (int j = 0; j < centriods[0].size(); j++)
            cout << centriods[i][j] << " ";
        cout << endl;
    }
